...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == "rpi/broadcast") {
      ...
      if messageTemp == "Starting detection")
        start =  millis();    
      if(messageTemp == "Object Detected"){
        long detect =  millis();
        objdetect_time = start - detect;
...
void loop() {
  ...
  if (now - lastMsg > 1000) {
    ...
    char msg [20]= {0};
    itoa(objectdetect_time, time_analysis, 10);
    msg = "Object detected in: " + time_analysis + " seconds."; **********??????????????/Probably wont work??????????*******************
    Serial.println(msg);
